...
ClickHouse is a columnar OLAP database built specifically for high-throughput append-only workloads — exactly what the analytics stream produces. Replacing the Redis Stream + `AnalyticsService` pipeline with ClickHouse resolves every sharding limitation described above.
...
- The ring buffer in `AnalyticsService` — replaced by a `GROUP BY toSecond(ts)` query scoped to the last N seconds.
...
The `/users/{id}/usage` endpoint, currently served by Redis hashes written by `AnalyticsService`, becomes:
...
The current implementation resolves user tier by `userId` prefix (`UserConfigService.getTier(userId)`). This is a deliberate simplification for a test task because it avoids introducing identity infrastructure while still exercising the full rate-limit rule engine.
...
**Implementation detail:** Both `allowTokenBucket` and `allowSlidingWindowCounter` in `RedisRateLimiterRepository` are annotated `@CircuitBreaker(name = "redisRateLimiter", fallbackMethod = "allowOnFailure")`. The fallback returns `true` unconditionally. This is an explicit availability-over-correctness trade-off: brief over-counting during an outage is preferable to denying all traffic with 5xx errors.
...
The codebase follows the conventional Spring Boot package structure used across most Spring projects:

- **controller** — REST controllers (`@RestController`). Depends on services and DTOs.
- **service** — Business logic (`@Service`). Contains `RateLimiterService`, `AnalyticsService`, `UserConfigService`, and algorithm strategies under `service/algorithm`.
- **repository** — Data access (`@Repository`). `RedisRateLimiterRepository` executes Lua scripts against Redis.
- **model** — Domain objects (enums, records, events). Pure Java with no framework dependencies.
- **dto** — Request/response data transfer objects.
- **config** — `@Configuration` classes, `@ConfigurationProperties`, and bean wiring.
- **event** — Redis Stream publisher and consumer for async analytics.

**What this enables:** Services can be unit-tested by subclassing concrete classes with test stubs. Algorithm strategies retain an interface (`RateLimitAlgorithm`) for the strategy pattern, while other dependencies are injected as concrete Spring-managed beans. Integration tests use Testcontainers to spin up a real Redis instance.

**Trade-off:** Without port interfaces, swapping implementations (e.g., replacing Redis with another store) requires modifying the repository class directly rather than adding a new adapter. This is acceptable for this project's scope and simplifies the codebase by removing indirection that only ever had one implementation.
